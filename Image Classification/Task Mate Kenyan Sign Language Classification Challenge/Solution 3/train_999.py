...
df_train['Labelv1'] = df_train['img_IDS'].map(relabels).fillna(df_train['Label'])
print(len(df_train[df_train.Labelv1!=df_train.Label]))
df_train['Label']=df_train['Labelv1']
df_train=df_train.drop(columns='Labelv1')

df_train['Labelv2'] = df_train['img_IDS'].map(relabelsv2).fillna(df_train['Label'])
print(len(df_train[df_train.Labelv2!=df_train.Label]))
df_train['Label']=df_train['Labelv2']
df_train=df_train.drop(columns='Labelv2')
...
def get_dls(df,fold_id,bs,size,max_size):
    ...
    dls = ImageDataLoaders.from_df(df[['path','code','is_valid']],'.',valid_col='is_valid',
                               ...
                               batch_tfms=[
                                          *aug_transforms(
                                             size=size,min_scale=0.8,
                                        do_flip=True,
                                        flip_vert=False,
                                        p_affine=0.75,
                                        p_lighting=0.75,
                                        mult=3.0,xtra_tfms=None,mode='bilinear',pad_mode='zeros'
                                        ),
...
size=224;max_size=320;
bs = 32
lr=1e-4

df,dls = get_dls(df_train,FOLD_ID,bs=bs,size=size,max_size=max_size)
...
learn = Learner(dls, model, cbs=[CutMix(1.),
                                 SaveModelCallback(monitor='valid_loss')],metrics=[accuracy])
with learn.no_bar():
    learn.fit_one_cycle(15,lr)
torch.save(learn.model.state_dict(), f'{MODEL_DIR}/model.pth')