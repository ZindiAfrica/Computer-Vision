    ...
    for grad, var in grads_and_vars:
        if grad is not None:
            if isinstance(grad, ops.IndexedSlices):
                ...
                grad_values = grad
            summaries.append(summary.histogram.histogram_summary(
...
def train(train_op,
    ...
    with graph.as_default():
        ...
        if fixed_random_seed:
            ...
            tf.set_random_seed(fixed_random_seed)

        if global_step is None:
            global_step = variables.get_or_create_global_step()
        saver = saver or tf_saver.Saver()

        # "step_saver" saves the model each "save_interval_steps" step (whereas "saver" saves each time interval)
        if save_interval_steps > 0:
            step_saver = tf.train.Saver(max_to_keep=0)
            os.makedirs(os.path.join(logdir, "step_checkpoints"), exist_ok=True)
            step_saver_path = os.path.join(logdir, "step_checkpoints", "model.ckpt")
        ...
        with ops.name_scope('init_ops'):
            ...
            if local_init_op == _USE_DEFAULT:
                ...
                except:  # in newer versions of TF, table_initializer is in lookup_ops
                    local_init_op = control_flow_ops.group(
                        tf_variables.local_variables_initializer(),
                ...
                try:
                    while not sv.should_stop():
                        ...
                        if should_stop:
                            ...
                            break
                        elif save_interval_steps > 0 and np_global_step % save_interval_steps == 0:
                            print('Saving at global step', np_global_step)
                            checkpoint_file = "%s-%d" % (step_saver_path, np_global_step)
                            step_saver.save(sess, checkpoint_file)
