...
from model import ConvGRUNet, SmallConvGRUNet
...
parser.add_argument('-bs','--best_submission', help='True for enabling training the model with the best configuration and False for the simpler one (default False)', default=False, type=bool)
...
for train_index, val_index in sss.split(areas[gts > -1], gts[gts > -1]):
    print(fold)
    fold += 1
    image_datasets = {'train': ICLRDataset(tile, imgs, areas, gts, field_masks, 'train', train_index, args.best_submission),
    ...
    if args.best_submission:
        ...
        model_ft = SmallConvGRUNet(imgs.shape[2]-1)
    ...
    model_ft_arr, _, _ = train_model_snapshot(model_ft, criterion, 0.008, dataloaders, dataset_sizes, device,
                           num_cycles=6 if args.best_submission else 5, num_epochs_per_cycle=10)
...
if args.best_submission:
    sub.to_csv(os.path.join(args.save_path, 'best_config_sub.csv'), index = False)
else:
    sub.to_csv(os.path.join(args.save_path, 'simple_config_sub.csv'), index = False)
