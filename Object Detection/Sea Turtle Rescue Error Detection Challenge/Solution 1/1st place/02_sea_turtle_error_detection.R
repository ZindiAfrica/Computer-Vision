...
# Contains the error detecting algorithms for the the following 4 columns:
# Release_Admiss_Notes, Date_Caught, ForagingGround and TurtleCharacteristics.
# The code was written by János Sávoly (a.k.a. CacoS)

pkgs <- c("mlr", "data.table", "tidyverse", "lubridate", "glue")
...
# Helper function (extracts the bycatch incidence number of a record).
extract_count <- function(rescue_id){
  str_match_all(rescue_id, "\\d{4}") %>%
    flatten_chr() %>%
    nth(2) %>% 
    as.integer()
}

extract_count_vec <- Vectorize(extract_count)
...
data <- dirty_data %>%
  ...
  mutate(Date_Caught = mdy(Date_Caught),
         ...
         ReleaseAdmissNotessbool = ifelse(is.na(Release_Admiss_Notes), 1, 0),
  ...
  select(Date_Caught_year,
         ...
         comp_release_land,
         Rescue_ID) %>%
  mutate(Rescue_number = extract_count_vec(Rescue_ID),
         Tag_2_minus_1 = Tag_2_01 - Tag_1_01,
  ...
  mutate_at(vars(ReleaseSite_01, LandingSite_01, Date_Caught_year),
            list(~as.integer(.))) %>%
  mutate_at(vars(Date_Caught_wday, CaptureMethod_01,
                 Species_01, Date_Caught_day, ForagingGround_01),
            list(~as.factor(.))) %>%
...
for (i in c(1, 4, 19, 22)){
  ...
  if (cur_target %in% c("Tag_3", "T_Number", "PCVNumber", "Expenditure")){
    tibble(response = rep(FALSE, 1362)) %>% write_csv(path = glue("prediction_1_{cur_target}.csv"))
  } else {
  train_01 <- data %>% slice(1:4292)
  train <- target %>% as_tibble() %>% select(cur_target) %>% cbind(train_01)
  ...
  ps <- makeParamSet(
    makeNumericParam(id = "nrounds", lower = log2(10/10), upper = log2(120/10), trafo = function(x) round(2^x * 10), default = log2(10/10)),
  ...
  tc <- makeTuneControlMBO(budget = 300, tune.threshold = TRUE)
  tr <- tuneParams(learner = lrn, task = tsk, resampling = rdesc, measures = logloss, 
